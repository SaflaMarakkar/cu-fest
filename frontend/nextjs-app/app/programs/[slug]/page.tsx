...
export default function ProgramDetail() {
  ...
  const fetchRegisteredUsersByProgramId = async () => {
    const res = await fetch(process.env.NEXT_PUBLIC_API_URL + "/users/registered/" + params.slug);
    const data = await res.json();
    if (!res.ok) {
      throw new Error("Failed to fetch programs");
    }
...
<div className="mt-8 flex justify-end">
      ...
      <section className="mt-12 flex flex-col items-center" ref={sectionRef}>
        ...
        <table className="table-auto w-screen-full text-gray-500 " style={{ borderCollapse: "collapse", marginTop: "1rem" }}>
              <thead>
                <tr>
          ...
          {registeredUsers?.map?.((user: any, index) => (
                <tr  key={user._id}>
                  ...
                  <td style={{ border: "1px solid #ddd", padding: "8px", textAlign: "left" }}>{user._id}</td>
                  <td style={{ border: "1px solid #ddd", padding: "8px", textAlign: "left" }}>{user.firstName}</td>
                  <td style={{ border: "1px solid #ddd", padding: "8px", textAlign: "left" }}>{user.lastName}</td>
